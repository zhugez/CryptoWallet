...
async def fetch_transaction(transaction_id: int, db: AsyncSession = Depends(get_db)):
...
async def transfer(transfer: TransferCreate, db: AsyncSession = Depends(get_db)):
    try:
